...
cp env.example .env
   #+end_src
3. Set strong passwords in the security section options of .env file by running the following bash script [[*Run It][section]]
   #+begin_src bash
./gen-passwords.sh
   #+end_src
...
5. Run docker-compose up -d if you want to ran Jibri and Jigase look this 
